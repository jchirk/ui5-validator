sap.ui.define([
	...
	var Validator = function () {
		...
		this._aPossibleAggregations = ["items", "content", "form", "formContainers", "formElements", "fields", "sections", "subSections",
			"_grid", "cells", "_page"
		];
	 ...
	 * @memberof nl.qualiture.plunk.demo.utils.Validator
	 */
	Validator.prototype.showMessage = function () {
	...
	Validator.prototype._validateRequired = function (oControl) {
		...
		for (var i = 0; i < this._aValidateProperties.length; i += 1) {
			try {
				...
				var oExternalValue = oControl.getProperty(this._aValidateProperties[i]);
				...
				if (!oExternalValue || oExternalValue === "") {
					...
					this._setValueState(oControl, ValueState.Error, "Please choose an entry!");
					isValid = false;
	...
	Validator.prototype._validateConstraint = function (oControl, i) {
		...
		if (editable) {
			try { // try validating the bound value
				var oControlBinding = oControl.getBinding(this._aValidateProperties[i]);
				var oExternalValue = oControl.getProperty(this._aValidateProperties[i]);
	...
	Validator.prototype._addMessage = function (oControl, sMessage) {
		var sLabel, eMessageType = MessageType.Error;
	...
	Validator.prototype._recursiveCall = function (oControl, fFunction) {
		for (var i = 0; i < this._aPossibleAggregations.length; i += 1) {
			...
			if (aControlAggregation instanceof Array) { // generally, aggregations are of type Array
